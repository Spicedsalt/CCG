...
double CWedgeOperator<M>::wedge_product()
{
    ...
    for (typename M::MeshFaceIterator fiter(m_pMesh[0]); !fiter.end(); ++fiter)
    {
        ...
        p += (du0[0] * (du1[1] - du1[2]) - du0[1] * (du1[0] - du1[2]) + du0[2] * (du1[0] - du1[1]))/6.0;
...
double CWedgeOperator<M>::wedge_star_product()
{
    ...
    for (typename M::MeshFaceIterator fiter(m_pMesh[0]); !fiter.end(); ++fiter)
    {
        ...
        //insert your code here to compute p from du0[3], du1[3];
        for (int i = 0; i < 3; i++) {
            p += 0.5 / std::tan(theta[i]) * du0[i] * du1[i];
           
        }
        /*p = (1 / 2) * (1 / std::tan(theta[0]) * du0[0] * du1[0] + 1 / std::tan(theta[1]) * du0[1] * du1[1] + 1 / std::tan(theta[2]) * du0[2] * du1[2])*/
    }
    //std::cerr << "wedge star product: " << p<< std::endl;
    return p;
};
